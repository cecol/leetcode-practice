...
public class LC546RemoveBoxes extends BasicTemplate {
   ...
   * [0-2]探討過且也記錄過, 然後有留下[0 to 2]間有元素等同於boxes[3] = 1的case, 共有 k == 2 個
   * 2. 這時候dp[i][j][k]有幾個選擇
   * 1. 先拿當前的k與i結算能拿到的基礎分數
   * 就是 (k + 1) * (k + 1) + rb(b, dp, i + 1, j, 0);
   * 因為我已知[0-2]以解完的子問題保有k個boxes[3] == 1, 再加上boxes[i]的情況
   * 有基本分數 k+1
  ...
  /**
   * 2021/1/8 發現上面的Top-down case 會得到Time Limit Exceeded, 不知道是否因為增加新的test case
   * 只有 bottom up case可以通過
   * 因此多加上bottom up case, 但尚未看懂 = =
   */
  public int removeBoxesBU(int[] boxes) {
    int n = boxes.length;
    int[][][] dp = new int[n][n][n];
    ...
    for (int left = 1; left < n; left++) {
      for (int right = left; right < n; right++) {
        int i = right - left; //不懂i為什麼是這樣決定

        for (int k = 0; k <= i; k++) {
          int res = (k + 1) * (k + 1) + dp[i + 1][right][0];

          for (int m = i + 1; m <= right; m++) { // 這段看起來很像top-down 
            if (boxes[m] == boxes[i]) {
              res = Math.max(res, dp[i + 1][m - 1][0] + dp[m][right][k + 1]);
            }
          }
